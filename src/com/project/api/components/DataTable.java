...
public class DataTable extends GridPane {
	...
	public void addTableColumn(Column column) {
		...
		if (String.class.equals(classDataType)) {
			tableColumns.add(tableFieldManager.getStringTableColumn(column
					.getColumnName()));
		} else if (Character.class.equals(classDataType)) {
			tableColumns.add(tableFieldManager.getCharacterTableColumn(column
					.getColumnName()));
		} else if (Double.class.equals(classDataType)) {
			tableColumns.add(tableFieldManager.getDoubleTableColumn(column
					.getColumnName()));
		} else if (Long.class.equals(classDataType)) {
			tableColumns.add(tableFieldManager.getLongTableColumn(column
					.getColumnName()));
		} else if (Date.class.equals(classDataType)) {
			tableColumns.add(tableFieldManager.getDateTableColumn(column
					.getColumnName()));
		}
	...
	private class Footer extends GridPane {
		...
		private ObservableList<Integer> noOfRecords = FXCollections
				.observableArrayList(5, 10, 50, 100, 200, 100);

		private Button firstButton = new Button(
				I18nUI.getString(Translations.FIRST));
		private Button previousButton = new Button(
				I18nUI.getString(Translations.PREVIOUS));
		private TextField currentPageField = new TextField();
		private Label totalPagesLabel = new Label();
		private Button nextButton = new Button(
				I18nUI.getString(Translations.NEXT));
		private Button lastButton = new Button(
				I18nUI.getString(Translations.LAST));
		private Label entriesLabel = new Label(
				I18nUI.getString(Translations.SHOWING_OUT_OF_ENTRIES));
		private Label pageSizeLabel = new Label(
				I18nUI.getString(Translations.PAGE_SIZE));
		...
		private void init() {
			...
			pageOptions.valueProperty().addListener(
		...
		private void loadButtonState(boolean first, boolean previous,
